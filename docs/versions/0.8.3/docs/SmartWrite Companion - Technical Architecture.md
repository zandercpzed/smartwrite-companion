...
Each feature is developed in a **separate file**. The main `SidebarView.ts` imports and composes these modules.

**Benefits:**

- Faster, more precise code edits
- Independent testing of each module
- Easier maintenance and debugging
- Smaller files = fewer merge conflicts

**Rule:** Never put multiple features in a single file. When in doubt, create a new module.
...
│   │
│   ├── personas/
│   │   ├── BasePersona.ts        # Abstract persona class
│   │   ├── PersonaFactory.ts     # Factory for creating personas
│   │   └── index.ts              # Persona definitions (embedded prompts)
│   │
│   ├── i18n/
...
class SuggestionEngine {
  constructor(private config: SuggestionConfig, private language: Language) {}
