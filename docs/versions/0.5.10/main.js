...
var PersonaPanel = class extends BasePanel {
  ...
  async performAnalysis(container, button) {
    ...
    if (resultsContainer) {
      ...
      loading.setText("\u{1F914} Analyzing & Creating Document...");
    }
    try {
...
## Output Format:
Provide 3-5 concise observations. Start each with a label (\u2713 Strength, \u26A0\uFE0F Issue, \u{1F4A1} Suggestion).

Example:
- \u2713 **Strength**: Opening hooks the reader effectively
- \u26A0\uFE0F **Issue**: Paragraph 3 introduces concepts not revisited later
- \u{1F4A1} **Suggestion**: Consider reordering sections 2 and 4 for better flow`
    });
...
Example:
- \u{1F60A} I really enjoyed how you explained [topic] - made total sense!
- \u{1F914} Lost me around paragraph 2 - too many technical terms
- \u{1F4A4} The middle section felt slow - almost skipped ahead
...
Example:
- \u{1F928} **Claim**: "Everyone prefers X" - Really? What about demographic Y?
- \u2696\uFE0F **Alternative View**: A critic might argue that Z is more important than X
- \u{1F50D} **Missing Evidence**: This assumes A causes B, but correlation \u2260 causation
...
Example:
- \u{1F525} **Hook Potential**: Opening line is FIRE - perfect for a TikTok!
- \u{1F4F8} **Quotable Moment**: "Insert quote here" - this will screenshot so well!
- \u{1F3AF} **Target Audience**: This will absolutely blow up with [demographic]
- \u{1F4A1} **Marketing Angle**: Pitch this as "[comparison]" meets "[comparison]"
- \u{1F4C9} **Concern**: Needs a stronger hook in first paragraph for scrollers`
    });
...
Example:
- \u{1F495} **Ship Alert**: The tension between A and B is *chef's kiss* - prime slowburn material!
- \u{1F3A8} **Fanart Gold**: That scene in paragraph 4 would make GORGEOUS fanart
- \u{1F4DD} **Headcanon**: Fans will definitely theorize about [character]'s backstory
- \u{1F525} **AU Potential**: This would be amazing as a [genre] AU
- \u{1F62D} **Emotional Impact**: The ending will destroy the fandom (in the best way)`
    });
...
You are a well-read literature enthusiast who has read extensively across genres. Your role is to compare the text with genre conventions, identify tropes, and spot clich\xE9s.
...
- **Clich\xE9s**: Spot overused phrases or plot devices
...
Example:
- \u{1F4DA} **Genre Context**: This reminds me of [Author]'s work, but with a [unique twist]
...
- \u{1F6AB} **Clich\xE9 Alert**: "It was a dark and stormy night" - consider subverting this
- \u{1F3AF} **Reference**: Strong echoes of [famous work] - intentional homage or coincidence?`
    });
  }
  /**
   ...
   */
  async analyzeText(personaId, text) {
...
// src/main.ts
var SmartWriteCompanionPlugin = class extends import_obsidian6.Plugin {
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on("editor-change", (0, import_obsidian6.debounce)((editor, view) => {
