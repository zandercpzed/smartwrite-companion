...
1. **Initial version:** 0.3.1
2. **Version increment:** On every code update, increment the last digit (e.g., 0.3.1 → 0.3.2)
3. **Testing workflow:** After every code update, update the plugin in Obsidian for user testing
4. **GitHub push:** Only push to GitHub after user confirms test is OK
5. **Modular architecture:** Each feature/phase must be developed in separate files. The main sidebar imports and composes these modules. This enables faster, more precise edits.
...
```
Clone and configure the SmartWrite Companion repository:

Repository: https://github.com/zandercpzed/smartwriting_companion
...
Never commit the docs/ folder or .DS_Store files.
```
...
### 1.2 Initialize Plugin

```
Create an Obsidian plugin project called "smartwrite-companion" with:
- TypeScript configuration
- esbuild for bundling
- manifest.json (id: smartwrite-companion, name: SmartWrite Companion, version: 0.3.1)
...
├── types.ts
├── ui/
│   ├── SidebarView.ts         # Main container - imports all panels
│   └── components/
...
```
Create the main sidebar view (SidebarView.ts):
- Extends ItemView
...
IMPORTANT: SidebarView only orchestrates. It imports and renders panels from separate files:
...
### 1.4 Modular Accordion UI

```
Implement modular accordion components for the sidebar.

Each module is a SEPARATE FILE in src/ui/components/:

1. SessionStatsPanel.ts - Session statistics
2. TextMetricsPanel.ts - Text metrics
3. ReadabilityPanel.ts - Readability scores
4. SuggestionsPanel.ts - Writing suggestions
5. PersonaPanel.ts - LLM persona analysis

Each panel file exports:
- A class extending a BasePanel
- render() method
- update(data) method
...
```
Create BasePanel.ts in src/ui/components/:

Abstract class that all panels extend:
...
This ensures consistent behavior across all panel modules.
```
