    ...
    private async performAnalysis(container: HTMLElement, button: HTMLButtonElement): Promise<void> {
        ...
        if (resultsContainer) {
            ...
            resultsContainer.createDiv({ 
                cls: 'smartwrite-stat-label', 
                attr: { style: 'margin-bottom: 8px;' } 
            }).setText('Status Analysis');

            // Stepper container
            const stepperDiv = resultsContainer.createDiv({ 
                cls: 'smartwrite-suggestion-description',
                attr: { style: 'text-align: left; padding: 16px; background: var(--background-secondary); border-radius: 6px;' }
            });
            
            const statusText = stepperDiv.createDiv({ text: 'Initializing...' });
            
            // Progress Bar
            const progressBar = stepperDiv.createDiv({ 
                cls: 'smartwrite-progress-bar',
                attr: { style: 'margin-top: 8px; width: 100%; height: 6px; background: var(--background-modifier-border); border-radius: 3px; overflow: hidden;' }
            });
            const progressFill = progressBar.createDiv({ 
                cls: 'smartwrite-progress-fill',
            ...
            try {
                ...
                errorDiv.setText('Analysis failed.');
