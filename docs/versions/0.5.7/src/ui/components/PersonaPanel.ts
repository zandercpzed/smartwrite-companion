...
export class PersonaPanel extends BasePanel {
    ...
    private async performAnalysis(container: HTMLElement, button: HTMLButtonElement): Promise<void> {
        ...
        const activeView = this.plugin.app.workspace.getActiveViewOfType(MarkdownView) as any;
        ...
        if (resultsContainer) {
            ...
            const loading = resultsContainer.createDiv({ 
                cls: 'smartwrite-suggestion-description',
        ...
        try {
            ...
            if (resultsContainer) {
                ...
                if (result.error) {
                    ...
                    const resultHeader = resultsContainer.createDiv({ 
                        cls: 'smartwrite-stat-label',
                        attr: { style: 'margin-bottom: 8px;' }
                    });
                    resultHeader.setText(`Analysis by ${result.personaName}`);

                    const resultContent = resultsContainer.createDiv({ 
                        cls: 'smartwrite-persona-analysis',
                        attr: { style: 'padding: 12px; background: var(--background-secondary); border-radius: 6px; white-space: pre-wrap; font-size: 12px; line-height: 1.6;' }
                    });
                    resultContent.setText(result.analysis);
                }
            }
            ...
            if (resultsContainer) {
                ...
                errorDiv.setText('Analysis failed. Please try again.');
            }
